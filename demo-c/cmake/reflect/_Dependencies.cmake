...
#
# Generated from sourcetree: 905D35FF-9C2E-4EE4-AB17-19D5983C1449;mulle-musl;no-all-load,no-cmake-loader,no-cmake-searchpath,no-import;
...
if( NOT MULLE_MUSL_LIBRARY)
   find_library( MULLE_MUSL_LIBRARY NAMES ${CMAKE_STATIC_LIBRARY_PREFIX}mulle-musl${CMAKE_DEBUG_POSTFIX}${CMAKE_STATIC_LIBRARY_SUFFIX} ${CMAKE_STATIC_LIBRARY_PREFIX}mulle-musl${CMAKE_STATIC_LIBRARY_SUFFIX} mulle-musl NO_CMAKE_SYSTEM_PATH NO_SYSTEM_ENVIRONMENT_PATH)
   ...
   if( MULLE_MUSL_LIBRARY)
      ...
      #
      foreach( _TMP_MULLE_MUSL_NAME "mulle-musl")
         set( _TMP_MULLE_MUSL_DIR "${_TMP_MULLE_MUSL_ROOT}/include/${_TMP_MULLE_MUSL_NAME}/cmake")
         # use explicit path to avoid "surprises"
         if( EXISTS "${_TMP_MULLE_MUSL_DIR}/DependenciesAndLibraries.cmake")
            ...
            set( NO_INHERIT_OBJC_LOADERS ON)
            #
            include( "${_TMP_MULLE_MUSL_DIR}/DependenciesAndLibraries.cmake")
            #
            list( GET _TMP_INHERIT_OBJC_LOADERS -1 NO_INHERIT_OBJC_LOADERS)
            list( REMOVE_AT _TMP_INHERIT_OBJC_LOADERS -1)
            #
            ...
            set( INHERITED_DEFINITIONS
               ${INHERITED_DEFINITIONS}
               ${MULLE_MUSL_DEFINITIONS}
               CACHE INTERNAL "need to cache this"
            )
            break()
         else()
            message( STATUS "${_TMP_MULLE_MUSL_DIR}/DependenciesAndLibraries.cmake not found")
         endif()
      endforeach()
   else()
...
#
# Generated from sourcetree: 69311EBF-FE80-4470-A4D3-AE6044C92649;mulle-musl-cc;no-all-load,no-cmake-loader,no-cmake-searchpath,no-import;
# Disable with : `mulle-sourcetree mark mulle-musl-cc no-link`
# Disable for this platform: `mulle-sourcetree mark mulle-musl-cc no-cmake-platform-${MULLE_UNAME}`
# Disable for a sdk: `mulle-sourcetree mark mulle-musl-cc no-cmake-sdk-<name>`
#
if( NOT MULLE_MUSL_CC_LIBRARY)
   find_library( MULLE_MUSL_CC_LIBRARY NAMES ${CMAKE_STATIC_LIBRARY_PREFIX}mulle-musl-cc${CMAKE_DEBUG_POSTFIX}${CMAKE_STATIC_LIBRARY_SUFFIX} ${CMAKE_STATIC_LIBRARY_PREFIX}mulle-musl-cc${CMAKE_STATIC_LIBRARY_SUFFIX} mulle-musl-cc NO_CMAKE_SYSTEM_PATH NO_SYSTEM_ENVIRONMENT_PATH)
   message( STATUS "MULLE_MUSL_CC_LIBRARY is ${MULLE_MUSL_CC_LIBRARY}")
   #
   # The order looks ascending, but due to the way this file is read
   # it ends up being descending, which is what we need.
   #
   if( MULLE_MUSL_CC_LIBRARY)
      #
      # Add MULLE_MUSL_CC_LIBRARY to DEPENDENCY_LIBRARIES list.
      # Disable with: `mulle-sourcetree mark mulle-musl-cc no-cmake-add`
      #
      set( DEPENDENCY_LIBRARIES
         ${DEPENDENCY_LIBRARIES}
         ${MULLE_MUSL_CC_LIBRARY}
         CACHE INTERNAL "need to cache this"
      )
      #
      # Inherit information from dependency.
      # Encompasses: no-cmake-searchpath,no-cmake-dependency,no-cmake-loader
      # Disable with: `mulle-sourcetree mark mulle-musl-cc no-cmake-inherit`
      #
      # temporarily expand CMAKE_MODULE_PATH
      get_filename_component( _TMP_MULLE_MUSL_CC_ROOT "${MULLE_MUSL_CC_LIBRARY}" DIRECTORY)
      get_filename_component( _TMP_MULLE_MUSL_CC_ROOT "${_TMP_MULLE_MUSL_CC_ROOT}" DIRECTORY)
      #
      #
      # Search for "DependenciesAndLibraries.cmake" to include.
      # Disable with: `mulle-sourcetree mark mulle-musl-cc no-cmake-dependency`
      #
      foreach( _TMP_MULLE_MUSL_CC_NAME "mulle-musl-cc")
         set( _TMP_MULLE_MUSL_CC_DIR "${_TMP_MULLE_MUSL_CC_ROOT}/include/${_TMP_MULLE_MUSL_CC_NAME}/cmake")
         # use explicit path to avoid "surprises"
         if( EXISTS "${_TMP_MULLE_MUSL_CC_DIR}/DependenciesAndLibraries.cmake")
            unset( MULLE_MUSL_CC_DEFINITIONS)
            list( INSERT CMAKE_MODULE_PATH 0 "${_TMP_MULLE_MUSL_CC_DIR}")
            # we only want top level INHERIT_OBJC_LOADERS, so disable them
            if( NOT NO_INHERIT_OBJC_LOADERS)
               set( NO_INHERIT_OBJC_LOADERS OFF)
            endif()
            ...
            set( NO_INHERIT_OBJC_LOADERS ON)
            #
            include( "${_TMP_MULLE_MUSL_CC_DIR}/DependenciesAndLibraries.cmake")
            #
            list( GET _TMP_INHERIT_OBJC_LOADERS -1 NO_INHERIT_OBJC_LOADERS)
            list( REMOVE_AT _TMP_INHERIT_OBJC_LOADERS -1)
            #
            list( REMOVE_ITEM CMAKE_MODULE_PATH "${_TMP_MULLE_MUSL_CC_DIR}")
            set( INHERITED_DEFINITIONS
               ${INHERITED_DEFINITIONS}
               ${MULLE_MUSL_CC_DEFINITIONS}
               CACHE INTERNAL "need to cache this"
            )
            break()
         else()
            message( STATUS "${_TMP_MULLE_MUSL_CC_DIR}/DependenciesAndLibraries.cmake not found")
         endif()
      endforeach()
   else()
      # Disable with: `mulle-sourcetree mark mulle-musl-cc no-require-link`
      message( FATAL_ERROR "MULLE_MUSL_CC_LIBRARY was not found")
   endif()
endif()
